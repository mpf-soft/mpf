...
class Helper extends \mpf\base\Helper
{
    ...
    /**
     * Options: return, echo, file.
     * If it's set as file then a location is needed
     * @var string|null
     */
    public $outputMethod = 'return';

    /**
     * If output is set to file then a location is needed here
     * @var string|null
     */
    public $outputLocation;

    /**
     * Skip os check for color and width
     * @var bool
     */
    public $skipOSCheck = false;

    protected $currentActionHadLogs, $pendingActionLogs = [], $currentAction;

    /**
     * Number of characters used to display last time
     * @var int
     */
    protected $lastTimeLenght;
     ...
     * @return string
     */
    protected function output(string $text): string
    {
        switch ($this->outputMethod) {
            case 'echo':
                ...
                return '';
            case 'file':
                file_put_contents($this->outputLocation, $text, FILE_APPEND);
                return '';
            default:
                return $text;
        }
     ...
     * @return string
     */
    public function color(string $message, string $color = self::CWHITE, string $background = null): string
    {
        if (self::OS_LINUX !== $this->OS) {
            return $message;
        }
        if (!($color || $background)) {
            return $message;
     ...
     * @return string
     */
    public function log(string $message, $color = self::CWHITE): string
    {
        return $this->output($this->color(date('Y-m-d H:i:s'), self::CLIGHT_GRAY) . $this->timeTextSeparator . $this->color($message, $color) . "\n");
    }
     ...
     * @param float $microtime a custom start time;
     * @return string
     */
    public function startAction($message, $color = self::CWHITE, float $microtime = null): string
    {
        $this->endAction();
        $this->endProgressBar();
    ...
    {
        $colorsList = ['0;30', '1;30', '0;34', '1;34', '0;32', '1;32', '0;36', '1;36', '0;31', '1;31', '0;35', '1;35', '0;33', '1;33', '0;37', '1;37', '40', '41', '42', '43', '44', '45', '46', '47', '0'];
     ...
     * @return string
     */
    public function continueAction(string $message, $color = 'default'): string
    {
     ...
     * @return string
     */
    public function endAction(string $message = 'done', string $color = self::CLIGHT_GREEN): string
    {
        $r = '';
        ...
        $time = $this->timeItTook($this->currentAction['time']);
     ...
     * @return int
     */
    protected function getScreen(string $what): int
    {
        ...
        if ($this->OS === self::OS_LINUX && ('file' !== $this->outputMethod)) {
            preg_match_all('/rows.(\d+);.columns.(\d+);/', strtolower(exec('stty -a  2> /dev/null | grep columns')), $output);
            if (count($output) === 3) {
                if (count($output[0])) {
     ...
     */
    public function timeItTook(float $startTime): string
    {
     ...
     * @return string
     */
    public function progressBar(int $total, int $done = 0, int $width = 200): string
    {
        if ($total <= 0 || 'file' === $this->outputMethod) {
            return '';
        }
        ...
        return $this->progress(null, $done);
    }

    public function progress(string $message = null, int $progressValue = 1, string $color = self::CWHITE): string
    {
        if ('file' === $this->outputMethod) {
            return '';
        }
        $line = '';
        if (null !== $message) {
            $message .= str_pad('', $this->getScreen('columns') - 19 - strlen($this->timeTextSeparator) - $this->getTextLength($message));
            $line = $this->log($message, $color);
        }
        ...
        $line .= ' ' . $this->color('[', $colorTotal);
        if ($fill && $fill < $this->activeProgressBar['width']) {
            if (($fill - 1) > 0) {
                $line .= $this->color(str_repeat('=', $fill - 1), $colorDone);
            }
            $line .= $this->color(str_repeat('=', 1), $colorCurrent);
        } else if ($fill > 0) {
            $line .= $this->color(str_repeat('=', $fill), $colorDone);
        }
        if (($this->activeProgressBar['width'] - $fill) > 0) {
            $line .= $this->color(str_repeat('-', $this->activeProgressBar['width'] - $fill), $colorTotal);
        }
        ...
        return $this->output($line . $this->color("] {$percent}% ({$done}/{$total}) {$remainingTime}\r", $colorTotal));
    }

    public function endProgressBar(): string
    {
     ...
     */
    public function logToHtml(string $logText, bool $userPre = true): string
    {
     ...
     * @return string
     */
    public function logToTextFile(string $logText): string
    {
     ...
     * @return string
     */
    public function input(string $text, string $defaultValue = '', string $color = null): string
    {
        $text .= ($defaultValue ? ": [$defaultValue]" : ': ');
        echo $color ? $this->color($text, $color) : $text;
        return trim(trim($result = fgets(fopen('php://stdin', 'rb'))) ? $result : $defaultValue);
    }
     ...
     * @return string
     */
    public function passwordInput(string $text): string
    {
